...
class RANSAC:
    def __init__(self, n=10, k=100, t=0.05, d=100, model=None, loss=None, metric=None):
...
if __name__ == "__main__":
    ...
    test_complete = np.load('/home/el_zlociako/Documents/AoR_CNN/Segmentacja/Images/test_seg.npy')
    # test_complete = test_complete[np.nonzero(test_complete)].flatten().reshape(-1, 1)
    test_complete[np.nonzero(test_complete)] = regressor.predict(line).reshape(1, -1)

    plt.plot(line, regressor.predict(line), c="r")

    elo1 = test_complete.flatten()
    ...
    plt.figure()
    plt.imshow(test_complete)
    np.save("WielkiTest.npy", test_complete)
